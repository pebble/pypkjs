...
class PebbleTimeline(object):
    ...
    def handle_resync(self, update_type, bin):
        for item in TimelineItem.select().where(TimelineItem.source_kind == 'web'):
            with model.db.atomic():
                ...
                if item.has_sent:
                    self.runner.pebble.blobdb.delete(item_id)
    ...
    def handle_pin_delete(self, pin):
        ...
        for child in pin.children:
            self.runner.pebble.blobdb.delete(types[child.type], pin_uuid)
            child.deleted = True
            child.save()
    ...
    def _send(self, item):
        ...
        if self._window_start() <= item.start_time < self._window_end():
            ...
            try:
                ...
                return
            self.runner.pebble.blobdb.insert(types[item.type], item.uuid, serialised, lambda x: self._did_send(item, x))
